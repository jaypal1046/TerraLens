...
public:
    ...
    // Getters for RAG/Knowledge Bridge
    const TransformerConfig& get_config() const { return config_; }
    const float* get_embedding(int token) const;
    
    // Loss calculation for Fact-Probing (Satellite Recon)
    float calculate_sequence_loss(const std::vector<int>& tokens, const std::vector<int>& targets);
...
private:
    ...
    // TerraLens Injection (Sandbox)
    float probe_radar(const std::vector<int>& tokens, const std::vector<int>& targets);
    void execute_skip(float skip_factor);
