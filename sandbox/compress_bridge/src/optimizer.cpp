...
void AdamOptimizer::adjust_lr_by_curvature(float curvature) {
    current_curvature_ = curvature;

    // 1. Acceleration (Wide Valleys)
    if (curvature > 0.0f && curvature < 1.0f) {
        learning_rate_ *= 1.1f; // Boost speed by 10%
        learning_rate_ = std::min(learning_rate_, 0.05f); // Cap
    } 
    // 2. Braking (Sharp Valleys)
    else if (curvature > 20.0f) {
        learning_rate_ *= 0.8f; // Drop speed by 20%
        learning_rate_ = std::max(learning_rate_, 1e-6f); // Floor
    }
    // 3. Mountains (Negative Curvature)
    else if (curvature < 0.0f) {
        learning_rate_ *= 0.5f; // Hard brake after a jump
    }
}
