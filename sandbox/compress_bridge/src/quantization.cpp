...
#ifdef __AVX2__
    // GOD_MODE: AVX2 implementation for 4-bit dot product
    // Processes 32 elements at a time (block size)
    int num_blocks = n / QK4_0;
    __m256 sum_vec = _mm256_setzero_ps();

    for (int b = 0; b < num_blocks; b++) {
        // Load scale (FP16 -> FP32)
        float scale_f = fp16_to_fp32(blocks[b].scale);
        __m256 scale = _mm256_set1_ps(scale_f);
        ...
        for (int i = 0; i < 32; i += 8) {
            ...
            // This is a simplified version; production-grade uses PSHUFB for faster nibble extraction
            float q_vals[8];
            for (int j = 0; j < 8; j += 2) {
                uint8_t packed = qs[(i+j)/2];
                q_vals[j] = (int8_t)(packed << 4) >> 4;
                q_vals[j+1] = (int8_t)(packed & 0xF0) >> 4;
            }
            
            __m256 q_vec = _mm256_loadu_ps(q_vals);
            __m256 x_vec = _mm256_loadu_ps(x_block + i);
            sum_vec = _mm256_fmadd_ps(_mm256_mul_ps(q_vec, scale), x_vec, sum_vec);
        }
    }
