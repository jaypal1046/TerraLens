...
void vec_axpy(float* y, const float* x, float a, int n) {
#ifdef __AVX2__
    if (g_cpu_features.avx2) {
...
void vec_rescale_and_add(float* dst, const float* src, float rescale, float weight, int n) {
#ifdef __AVX2__
    if (g_cpu_features.avx2) {
        __m256 r_vec = _mm256_set1_ps(rescale);
        ...
        for (; i + 7 < n; i += 8) {
            ...
            // result = d*r + s*w
            __m256 result = _mm256_add_ps(_mm256_mul_ps(d_vec, r_vec), _mm256_mul_ps(s_vec, w_vec));
            _mm256_storeu_ps(&dst[i], result);
        }
        for (; i < n; i++) {
            dst[i] = dst[i] * rescale + src[i] * weight;
        }
        return;
    }
#endif
