...
bool ans_build_table(const uint32_t raw_freq[256], AnsTable& table) {
    ...
    // Fix rounding error by distributing slack using the largest remainder method.
    // This minimizes the distortion of the original probabilities.
    if (used != ANS_M) {
        struct SR { int sym; int64_t rem; };
        SR rems[256]; int nr = 0;
        for (int s = 0; s < 256; ++s) {
            if (raw_freq[s] == 0) continue;
            // rem = (ideal_scaled * total) - (actual_scaled * total)
            rems[nr++] = {s, (int64_t)raw_freq[s] * ANS_M - (int64_t)scaled[s] * total};
        }

        if (used < ANS_M) {
            // Under-allocated: add slots to symbols with largest remainders
            std::sort(rems, rems + nr, [](const SR& a, const SR& b){ return a.rem > b.rem; });
            for (int i = 0; i < (int)(ANS_M - used) && i < nr; ++i)
                scaled[rems[i].sym]++;
        } else {
            // Over-allocated: remove slots from symbols with smallest remainders (most negative deviation)
            // but ensure scaled[s] remains at least 1.
            std::sort(rems, rems + nr, [](const SR& a, const SR& b){ return a.rem < b.rem; });
            int to_remove = (int)(used - ANS_M);
            for (int i = 0; i < nr && to_remove > 0; ++i) {
                if (scaled[rems[i].sym] > 1) {
                    scaled[rems[i].sym]--;
                    to_remove--;
                }
            }
...
static inline uint8_t ctx2_key(uint8_t prev2, uint8_t prev1) {
    // Improved context folding: uses a simple bit-mix to reduce collisions
    // compared to a basic linear multiplier.
    uint32_t h = (uint32_t)prev2 ^ ((uint32_t)prev1 << 4);
    h ^= h >> 3;
