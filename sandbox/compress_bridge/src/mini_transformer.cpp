...
std::vector<std::vector<float>> MiniTransformer::test_forward(
    const std::vector<int>& tokens,
    MixedPrecision::MixedPrecisionOptimizer::PrecisionMode mode
) {
    auto x = forward(tokens, mode);
    int seq_len = x.size();
    if (seq_len == 0) return {};

    std::vector<std::vector<float>> logits(seq_len, std::vector<float>(config_.vocab_size, 0.0f));
    for (int i = 0; i < seq_len; i++) {
        for (int v = 0; v < config_.vocab_size; v++) {
            float sum = 0;
            for (int j = 0; j < config_.embedding_dim; j++) {
                sum += x[i][j] * weights_.output_projection[j][v];
            }
            logits[i][v] = sum;
        }
    }
...
void MiniTransformer::train(
    ...
    // TERRALENS PHASE 2: Satellite Warm-Start
    if (!sequences.empty()) {
        std::cerr << "[TERRALENS] 🛰️ Initiating Phase 2 Satellite Reconnaissance...\n";
        TerraLens::SatelliteScanner scanner(*this);
        const auto& seq = sequences[0];
        std::vector<int> warm_in(seq.begin(), seq.end()-1);
        std::vector<int> warm_tgt(seq.begin()+1, seq.end());
        scanner.scan(warm_in, warm_tgt, 20); // Perform 20 candidate sweep
        scanner.apply_best_basin();
    }
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (size_t i = 0; i < sequences.size(); i += batch_size) {
            ...
            // TERRALENS INJECTION: Every 50 batches, probe the landscape
            if (batch_count % 50 == 0) {
                const auto& seq = sequences[i % sequences.size()];
                std::vector<int> probe_in(seq.begin(), seq.end()-1);
                std::vector<int> probe_tgt(seq.begin()+1, seq.end());
                
                float curvature = probe_radar(probe_in, probe_tgt);
                
                // Adjust Learning Rate based on curvature
                optimizer.adjust_lr_by_curvature(curvature);
                
                if (curvature < -0.05f) {
                    std::cerr << "  [TERRALENS] Non-convexity detected (Curv: " << curvature << "). Executing Skip!\n";
                    execute_skip(curvature); // Pass curvature for dynamic skip
                }
            }
...
// ============================================================================
// TERRALENS IMPLEMENTATION (SANDBOX)
// ============================================================================

float MiniTransformer::probe_radar(const std::vector<int>& tokens, const std::vector<int>& targets) {
    // 1. Get baseline loss
    std::vector<LayerCache> caches;
    auto x = forward_with_cache(tokens, caches);
    // Simple loss calculation for probe
    float L0 = 0.0f;
    for (size_t i = 0; i < std::min(tokens.size(), targets.size()); i++) {
        L0 += std::abs(x[i][0] - targets[i]); // Proxy loss
    }

    // 2. Perturb and measure (Numerical curvature)
    float h = 1e-4f;
    // Perturb first embedding weight as a proxy for landscape complexity
    weights_.token_embeddings[0][0] += h;
    auto x_plus = forward_with_cache(tokens, caches);
    float L_plus = 0.0f;
    for (size_t i = 0; i < std::min(tokens.size(), targets.size()); i++) L_plus += std::abs(x_plus[i][0] - targets[i]);
    
    weights_.token_embeddings[0][0] -= 2*h;
    auto x_minus = forward_with_cache(tokens, caches);
    float L_minus = 0.0f;
    for (size_t i = 0; i < std::min(tokens.size(), targets.size()); i++) L_minus += std::abs(x_minus[i][0] - targets[i]);

    weights_.token_embeddings[0][0] += h; // Restore

    // Second derivative estimate
    return (L_plus - 2*L0 + L_minus) / (h * h);
}

void MiniTransformer::execute_skip(float curvature) {
    // Dynamic skip magnitude: steeper mountains require bigger jumps
    // We use log of absolute curvature to scale the jump reasonably
    float skip_magnitude = std::min(10.0f, std::abs(curvature) * 0.1f);
    
    std::cerr << "  [TERRALENS] Dynamic Skip Magnitude: " << skip_magnitude << "\n";

    // Jump embeddings and output head
    for (auto& row : weights_.token_embeddings) {
        for (float& w : row) w += (float(rand())/RAND_MAX - 0.5f) * skip_magnitude * 0.01f;
    }
    for (auto& row : weights_.output_projection) {
        for (float& w : row) w += (float(rand())/RAND_MAX - 0.5f) * skip_magnitude * 0.01f;
    }
}

const float* MiniTransformer::get_embedding(int token) const {
    if (token < 0 || token >= static_cast<int>(weights_.token_embeddings.size())) {
        return weights_.token_embeddings[0].data();
    }
    return weights_.token_embeddings[token].data();
}

float MiniTransformer::calculate_sequence_loss(const std::vector<int>& tokens, const std::vector<int>& targets) {
    auto logits = forward(tokens);
    float total_loss = 0;
    
    for (size_t i = 0; i < std::min(logits.size(), targets.size()); i++) {
        // Cross Entropy: -log(softmax(logits[i][targets[i]]))
        float sum_exp = 1e-9f;
        for (float l : logits[i]) sum_exp += std::exp(l);
        
        int target_idx = targets[i];
        if (target_idx < 0 || target_idx >= static_cast<int>(logits[i].size())) continue;
        
        float target_logit = logits[i][target_idx];
        total_loss += - (target_logit - std::log(sum_exp));
    }
